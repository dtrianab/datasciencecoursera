The variables included in the final tidy dataset "data_clean_DT.txt" are:
...
Original data Read, then it Merges the training and the test sets to create one data set. Applying row binding rbind() to test and train variables as well to subjects data. Extracts only the measurements on the mean and standard deviation for each measurement. Convert variable feature as character and the filter it by mean or std key words in order to find the variables. The indices are stored into a variable, it is used with select() fucntion for slicing the columns.
